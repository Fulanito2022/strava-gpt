...
def _ensure_valid_access_token(athlete_id: int) -> str:
    ...
    expires_s = _epoch_s(tok.expires_at)  # soporta int/datetime/str
...
@app.get("/")
def root():
    return {"ok": True, "message": "Strava GPT backend activo"}


@app.get("/admin/health")
def admin_health(request: Request, db=Depends(get_db)):
    _auth_admin_or_403(request)
    # Si se inyecta la sesión sin error, consideramos DB OK.
...
def refresh_token(request: Request, athlete_id: Optional[int] = None):
    ...
    return {
        ...
        "access_token_tail": (data.get("access_token") or "")[-6:],
    }
